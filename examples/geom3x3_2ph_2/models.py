...
class NavierStokes2DwSat(ForwardIVP):
    ...
    def neural_net(self, params, t, x, y):
        ...
        B1 = jax.random.normal(subkey, (2, 2)) *1
        key, subkey = jax.random.split(key)
        B2 = jax.random.normal(subkey, (2, 2)) *10
        
        X = jnp.stack([x, y])
        X = jnp.transpose(X)
       
        X1 = jnp.concatenate((jnp.sin(X@B1), jnp.cos(X@B1)), axis=0)
        X2 = jnp.concatenate((jnp.sin(X@B2), jnp.cos(X@B2)), axis=0)
        
        X = jnp.stack([t, x, y])
        X = jnp.transpose(X)
        # t=jnp.stack([t])
        
        inputs = jnp.concatenate((X, X1, X2), axis =0)
        # inputs = jnp.concatenate((t, X1), axis =0)
            
        # inputs = jnp.stack([t, jnp.transpose(X1), jnp.transpose(X2)])
    ...
    def r_net(self, params, t, x, y):
        ...
        # PDE residual
        print(f'Reynolds = {Re}')
        # print(f'u_t = {jnp.sqrt(u_t**2)}')
        # print(f'u * u_x + v * u_y = {u * u_x + v * u_y}')
        # print(f'p_x/re = { (1/self.p_factor * p_x ) / Re}')
        # print(f'(u_xx + u_yy)/re = { (- mu_ratio*(v_xx + v_yy)) / Re}')
        ...
        return ru/(10**0), rv/(10**0), rc, rs #4.8
