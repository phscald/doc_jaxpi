...
class NavierStokes2DwSat(ForwardIVP):
    ...
    def __linear_scaler_equation(self, mu, mus, muq, mur):
        return (mur-1)/(mus-muq) * (mu-muq) +1
    
    def __nonlinear_scaler_equation(self, mu, mus, muq, mur):
        a = mur / ( (mus**(-2) - muq**(-2)) +1 )
        b = 1- a * muq**(-2)
        return a*mu**(-2) +b
    ...
    def __linear_scaler(self, mu):
        ...
        yu =  self.__linear_scaler_equation(mu, mus, muq, mur)    
        # yv =  self.__linear_scaler_equation(mu, mus, muq, mur) # 54/40              
    ...
    def __nonlinear_scaler(self, mu):
        ...
        yu =  self.__nonlinear_scaler_equation(mu, mus, muq, mur)    
        # yv =  self.__nonlinear_scaler_equation(mu, mus, muq, mur)                  
        # return 0.00174*yu, 0.00027111*yv
        return yu
    
    def __scale_function(self, funct, old_scale, new_scale):
        funct = funct -1 
        funct = funct/(old_scale-1)
        funct = funct*(new_scale-1)
        return funct+1

    def neural_net(self, params, t, x, y, mu):
        # scaler = self.__nonlinear_scaler(mu)
        # scaler = self.__scale_function(scaler, old_scale=.1/.0025, new_scale=16)
        scaler = 1
        ...
        s = outputs[3]
        # return u, v, p, s
        return u*scaler*0.00174, v*scaler*0.00027111, p, s # nn.sigmoid(s)
    # u*0.01, v*0.001, p
    ...
    def r_net(self, params, t, x, y, mu0):
        ...
        u, v, p, s = self.neural_net(params, t, x, y, mu0)
    ...
    def losses(self, params, batch):
        ...
        if self.config.weighting.use_causal == True:
            ru_l, rv_l, rc_l, rs_l, gamma = self.res_and_w(params, 
                                                           jnp.concatenate((res_batch, noslip_batch), axis=0)
                                                           )
            ...
            # ru_loss = jnp.mean(jnp.concatenate((ru_pred, ru_pred2), axis=0)**2)
            # rv_loss = jnp.mean(jnp.concatenate((rv_pred, rv_pred2), axis=0)**2)
            # rc_loss = jnp.mean(jnp.concatenate((rc_pred, rc_pred2), axis=0)**2)
            # rs_loss = jnp.mean(jnp.concatenate((rs_pred, rs_pred2), axis=0)**2)
