...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    U_max = 1.6#17#.25/3#visc .1
    # pmax = 15

    L_max = .021
    pmax = mu*U_max/L_max*798
    ...
    if config.nondim == True:
        ...
        u_clim = (jnp.min(u_ref), jnp.max(u_ref))
        v_clim = (jnp.min(v_ref), jnp.max(v_ref))
        p_clim = (jnp.min(p_ref), jnp.max(p_ref))
